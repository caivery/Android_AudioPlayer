...
public class MP3TagWriter extends AudioTagWriter {
	...
	private final static List<FieldKey> trackDiscFieldKeys = new ArrayList<FieldKey>(
			Arrays.asList(FieldKey.TRACK, FieldKey.TRACK_TOTAL,
	...
	public void write(TrackData trackData) throws TagWriteException {
		...
		TextEncoding.getInstanceOf().setDefaultNonUnicode(
				AudioFileReader.getDefaultCharset().name());

		if (/* song.getCustomHeaderField("hasApeTag") != null */false) {
			try {
				apeTagProcessor.writeAPEv2Tag(trackData);
			} catch (IOException e) {
				e.printStackTrace();
			}
		} else {
			...
			try {
				...
				copyTagFields(id3v2tag, trackData);
	...
	public void copyTagFields(ID3v24Tag tag, TrackData trackData)
	...
	private void handleTrackDiscFields(ID3v24Tag tag, TrackData trackData)
